...
class GDN_1(nn.Module):
    def __init__(self, feats, sampling_rate=1.0):
        super(GDN_1, self).__init__()
        self.name = 'GDN_1'
        self.lr = 0.0001
        self.n_feats = feats
        self.n_window = 5
        self.n_hidden = 16
        self.n = self.n_window * self.n_feats
        self.sampling_rate = sampling_rate  # 노이즈 적용 비율
    ...
    def apply_noise(self, z):
        # Gaussian noise 생성
        mean = torch.mean(z, dim=0, keepdim=True)
        std = torch.std(z, dim=0, keepdim=True)
        gaussian_noise = torch.normal(mean, std).expand_as(z)

        # 노이즈 적용
        if self.sampling_rate < 1.0:
            mask = torch.rand_like(z) < self.sampling_rate
            z_with_noise = z.clone()
            z_with_noise[mask] -= gaussian_noise[mask]
        else:
            z_with_noise = z - gaussian_noise  # sampling_rate == 1.0일 때, 모든 노드에 노이즈 적용
    ...
    def forward(self, data):
        ...
        # 노드 표현(zi)에 노이즈 적용
        feat_r = self.apply_noise(feat_r)
...
class TranAD_1(nn.Module):
    def __init__(self, feats):
        super(TranAD_1, self).__init__()
        self.name = 'TranAD_1'
        self.lr = lr
        self.batch = 128
        self.n_feats = feats
    ...
    def encode(self, src, c, tgt):
        ...
        # Gaussian noise 제거 적용
        mean = torch.mean(memory, dim=0, keepdim=True)  # 전체 memory 텐서의 평균
        std = torch.std(memory, dim=0, keepdim=True)    # 전체 memory 텐서의 표준편차
        gaussian_noise = torch.normal(mean, std)        # Gaussian 노이즈 생성
        memory = memory - gaussian_noise                # 노이즈 제거
...
class TranAD_2(nn.Module):
    def __init__(self, feats, sampling_rate=1.0):
        super(TranAD_2, self).__init__()
        self.name = 'TranAD_2'
        self.lr = lr
        self.batch = 128
        self.n_feats = feats
    ...
    def encode(self, src, c, tgt):
        ...
        # Gaussian noise 생성 및 제거 적용
        mean = torch.mean(memory, dim=0, keepdim=True)  # 전체 memory 텐서의 평균
        std = torch.std(memory, dim=0, keepdim=True)    # 전체 memory 텐서의 표준편차
        gaussian_noise = torch.normal(mean, std)        # Gaussian 노이즈 생성
        gaussian_noise = gaussian_noise.expand_as(memory)  # batch 차원 확장
        
        if self.sampling_rate < 1.0:
            mask = torch.rand_like(memory) < self.sampling_rate  # 노이즈 적용 비율
            memory_with_noise = memory.clone()
            memory_with_noise[mask] -= gaussian_noise[mask]  # 비율에 따라 노이즈 적용
        else:
            memory_with_noise = memory - gaussian_noise  # sampling_rate == 1일 경우, 전체 노이즈 제거

        tgt = tgt.repeat(1, 1, 2)
        return tgt, memory_with_noise
